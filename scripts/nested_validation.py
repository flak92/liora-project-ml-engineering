...
def run_procedure(dfx, dfb, tev, bnds_inner, ticker, cands, seed):
    ...
    base_folds, base_trades = FS.eval_subset(dfx, dfb, tev, bnds_inner, best_params,
                                             FS._names(ticker, []), seed)
    ...
    survivors = []
    for cid in cands:
        folds, _ = FS.eval_subset(dfx, dfb, tev, bnds_inner, best_params,
                                  FS._names(ticker, [cid]), seed)
    ...
    def try_add(cid):
        folds, _ = FS.eval_subset(dfx, dfb, tev, bnds_inner, best_params,
                                  FS._names(ticker, state["selected"] + [cid]), seed)
        mean = FS._fmean(folds)
        if mean is None:
            return False
...
def theta_on_inner(dfx, dfb, tev, bnds_inner, params, names, seed):
    ...
    fold_data = []
    for tr, va in P.purged_wf_folds(t0s, bnds_inner["train_start_idx"], bnds_inner["train_end_idx"]):
    ...
    if not fold_data:
        return None
    return float(P.score_shared_operating_point(dfx, fold_data)["theta"])


def score_outer(dfx, dfb, tev, names, params, theta, tr, va, seed):
    """One evaluation on the untouched outer fold, at the operating point fixed by the inner run."""
    import numpy as np
    import xgboost as xgb
    import pipeline as P
    ...
    grid = P.op_grid_scores(dfx, [(scored, min(vt0), max(vt0))], [float(theta)], P.op_lambdas())
    if not grid:
        return None, 0
    g = grid[0]
    return float(sum(g["fold_growth"])), int(sum(g["fold_trades"]))


def nested_table(ticker, seed=42):
    import feature_search as FS
    import pipeline as P
    ...
    for i, (tr, va, inner_end) in enumerate(folds):
        ...
        proc = run_procedure(dfx, dfb, tev, bnds_inner, ticker, cands, seed)
        core = FS._names(ticker, [])
        picked = FS._names(ticker, proc["selected"]) if proc["selected"] else core

        th_core = theta_on_inner(dfx, dfb, tev, bnds_inner, proc["params"], core, seed)
        th_pick = (theta_on_inner(dfx, dfb, tev, bnds_inner, proc["params"], picked, seed)
                   if proc["selected"] else th_core)
        g_core, n_core = ((None, 0) if th_core is None else
                          score_outer(dfx, dfb, tev, core, proc["params"], th_core, tr, va, seed))
        g_pick, n_pick = ((None, 0) if th_pick is None else
                          score_outer(dfx, dfb, tev, picked, proc["params"], th_pick, tr, va, seed))
        ...
        g_pick_ct, n_pick_ct = ((None, 0) if (th_core is None or not proc["selected"]) else
    ...
    return {"ticker": ticker, "outer_folds": len(folds), "seconds": round(time.time() - t0, 1),
            "folds": out}
...
def main():
    ...
    if args.jobs > 1:
        ...
        with ProcessPoolExecutor(max_workers=args.jobs) as ex:
            for r in ex.map(nested_table, tickers):
                results[r["ticker"]] = r
    ...
    Path(args.out).write_text(json.dumps({"k_out": K_OUT, "tables": results}, indent=1) + "\n",
                              encoding="utf-8")
    print(f"\nwrote {args.out}")
    return 0
